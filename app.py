...
# === Config ===
load_dotenv()
GROQ_API_KEY = os.getenv('GROQ_API_KEY', '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')
NEO4J_URI = os.getenv('NEO4J_URI', 'neo4j://localhost:7687')
NEO4J_USERNAME = os.getenv('NEO4J_USERNAME', 'neo4j')
NEO4J_PASSWORD = os.getenv('NEO4J_PASSWORD', 'Rohit@17')

if not GROQ_API_KEY:
    raise ValueError("GROQ_API_KEY not set in environment variables.")

app = Flask(__name__)
# Update CORS to allow multiple origins (adjust the port range as needed)
...
# Initialize Neo4j driver with more detailed logging
@retry(
...
def init_neo4j_driver():
    logger.info(f"Attempting to connect to Neo4j at {NEO4J_URI} with username {NEO4J_USERNAME}")
    try:
...
def shutdown_server():
    ...
    time.sleep(1)  # Add a small delay to allow in-flight requests to complete
    neo4j_driver.close()
